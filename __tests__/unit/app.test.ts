...
describe('App Controller & Logic Service', () => {
  ...
  describe('handleGetReportRequestByMsgId', () => {
    it('should handle happy path', async () => {
      ...
      databaseManager._transaction.query = mockQueryFn.mockResolvedValue({
        batches: {
          all: mockBatchesAllFn,
        },
      });
      jest.spyOn(databaseManager._transaction, 'query');

      const handleSpy = jest.spyOn(LogicService, 'handleGetReportRequestByMsgId');
      await LogicService.handleGetReportRequestByMsgId('');
